...
describe("Managers", function () {
    ...
    describe("Tax Manager", function () {
        ...
        it("Tax manager should be deployed, and initialized", async function () {
            ...
            expect(await contracts.taxManager.custodian()).to.equal(await accounts.owner.getAddress());
        ...
        it("Only custodians should be able to set the custodian", async function () {
            ...
            await expect(taxManager.connect(accounts_.seeker).setCustodian(newCustodian)).to.be.revertedWith("only custodian");
            await taxManager.connect(accounts_.owner).setCustodian(newCustodian);
            expect(await taxManager.custodian()).to.equal(newCustodian);

            await taxManager.connect(accounts_.seeker).setCustodian(await accounts_.owner.getAddress());
        });
        ...
        it("Only custodians should be able to set the platform treasury pool", async function () {
            ...
            await expect(taxManager.setPlatformTreasuryPool(ethers.ZeroAddress)).to.be.revertedWith("Zero address");
            await expect(taxManager.connect(accounts_.seeker).setPlatformTreasuryPool(newPool)).to.be.revertedWith("only custodian");
            await taxManager.connect(accounts_.owner).setPlatformTreasuryPool(newPool);
        ...
        it("only custodians should be able to set the platform revenue pool", async function () {
            const newPool = await accounts_.seeker.getAddress();
            await expect(taxManager.setPlatformRevenuePool(ethers.ZeroAddress)).to.be.revertedWith("Zero address");
            await expect(taxManager.connect(accounts_.seeker).setPlatformRevenuePool(newPool)).to.be.revertedWith("only custodian");
            await taxManager.connect(accounts_.owner).setPlatformRevenuePool(newPool);
        ...
        it("Only custodians should be able to set the referral tax treasury", async function () {
            const newPool = await accounts_.seeker.getAddress();
            await expect(taxManager.setReferralTaxTreasury(ethers.ZeroAddress)).to.be.revertedWith("Zero address");
            await expect(taxManager.connect(accounts_.seeker).setReferralTaxTreasury(newPool)).to.be.revertedWith("only custodian");
            await taxManager.connect(accounts_.owner).setReferralTaxTreasury(newPool);
        ...
        it("only custodians should be able to set the disputeFeesTreasury", async function () {
            const newPool = await accounts_.seeker.getAddress();
            await expect(taxManager.setDisputeFeesTreasury(ethers.ZeroAddress)).to.be.revertedWith("Zero address");
            await expect(taxManager.connect(accounts_.seeker).setDisputeFeesTreasury(newPool)).to.be.revertedWith("only custodian");
            await taxManager.connect(accounts_.owner).setDisputeFeesTreasury(newPool);
        ...
        it("Only custodian should be able to set the seeker fees within the valid tax rate", async function () {
            ...
            await expect(taxManager.connect(accounts_.solver).setSeekerFees(seekerReferralRewards, seekerPlatformrevenue)).to.be.revertedWith("only custodian");

            await taxManager.setSeekerFees(seekerReferralRewards, seekerPlatformrevenue);
        ...
        it("Will revert on invalid tax rate on set seeker fees", async function () {
            ...
            await expect(taxManager.setSeekerFees(seekerReferralRewards, seekerPlatformrevenue)).to.be.revertedWith("Tax rate too high");
        });
        ...
        it("Will revert on invalid tax rate on set solver fees", async function () {
            ...
            await expect(taxManager.setSolverFees(solverReferralTax, solverPlatformRev, solverPlatformTreasury)).to.be.revertedWith("Tax rate too high");
        });
        ...
        it("Will revert on invalid tax rate on set bulk referral rate", async function () {
            ...
            await expect(taxManager.setBulkReferralRate(1, first, second, third, fourth)).to.be.revertedWith("Tax rate too high");
        });

        it("Will revert on invalid tax rate on set dispute rate", async function () {
            const disputeDepositRate = 1000000;

            await expect(taxManager.setDisputeDepositRate(disputeDepositRate)).to.be.revertedWith("Tax rate too high");
        });
        ...
        it("Only custodian should be able to set the solver fees within the valid tax rate", async function () {
            ...
            await expect(taxManager.connect(accounts_.solver).setSolverFees(solverReferralRewards, solverPlatformrevenue, solverPlatformTreasury)).to.be.revertedWith("only custodian");


            await taxManager.setSolverFees(solverReferralRewards, solverPlatformrevenue, solverPlatformTreasury);
        ...
        it("Only custodian should be able to set bulkReferralRate", async function () {
            await expect(taxManager.connect(accounts_.solver).setBulkReferralRate(1, 100, 200, 300, 400)).to.be.revertedWith("only custodian");
        ...
        it("Only custodian should be able to set disutedepositrate", async function () {
            await expect(taxManager.connect(accounts_.solver).setDisputeDepositRate(100)).to.be.revertedWith("only custodian");
        ...
        it("Only custodian should be able to recoverTokens native", async function () {
            await expect(taxManager.connect(accounts_.solver).recoverTokens(ethers.ZeroAddress, await accounts_.solver.getAddress())).to.be.revertedWith("only custodian");
            ...
            expect(await ethers.provider.getBalance(taxManager.target)).to.equal(ethers.parseEther("1.0"));

            const balanceBefore = await ethers.provider.getBalance(await accounts_.solver.getAddress());

            await taxManager.recoverTokens(ethers.ZeroAddress, await accounts_.solver.getAddress());
        ...
        it("Only custodian should be able to recoverTokens token", async function () {
            ...
            await taxManager.recoverTokens(mockToken.target, await accounts_.solver.getAddress());
    ...
    describe("TierManager", function () {
        ...
        it("TierManager should be deployed, and initialized", async function () {
            ...
            expect(await contracts.tierManager.magistrate()).to.equal(await accounts.owner.getAddress());
            expect(await contracts.tierManager.xpToken()).to.equal(contracts.xpToken.target);
        ...
        it("Only magistrate should be able to set the magistrate", async function () {
            await expect(tierManager.connect(accounts_.seeker).setMagistrate(await accounts_.seeker.getAddress())).to.be.revertedWith("only magistrate");

            await tierManager.setMagistrate(await accounts_.seeker.getAddress());
            expect(await tierManager.magistrate()).to.equal(await accounts_.seeker.getAddress());

            await tierManager.connect(accounts_.seeker).setMagistrate(await accounts_.owner.getAddress());
        });

        it("Only magistrate should be able to set the xpToken", async function () {
            await expect(tierManager.connect(accounts_.seeker).setXpToken(await accounts_.seeker.getAddress())).to.be.revertedWith("only magistrate");

            await tierManager.setXpToken(await accounts_.seeker.getAddress());
            expect(await tierManager.xpToken()).to.equal(await accounts_.seeker.getAddress());

            await tierManager.setXpToken(xpToken.target);
        });

        it("Only magistrate should be able to set the tier", async function () {
            await expect(tierManager.connect(accounts_.seeker).setConditions(1, 1, 1, 1, 1, 1)).to.be.revertedWith("only magistrate");
        ...
        it("Only magistrate should be able to recoverTokens native", async function () {
            await expect(tierManager.connect(accounts_.solver).recoverTokens(ethers.ZeroAddress, await accounts_.solver.getAddress())).to.be.revertedWith("only magistrate");
            ...
            expect(await ethers.provider.getBalance(tierManager.target)).to.equal(ethers.parseEther("1.0"));

            const balanceBefore = await ethers.provider.getBalance(await accounts_.solver.getAddress());

            await tierManager.recoverTokens(ethers.ZeroAddress, await accounts_.solver.getAddress());
        ...
        it("Only magistrate should be able to recoverTokens token", async function () {
            ...
            expect(await mockToken.balanceOf(tierManager.target)).to.equal(1000);

            await tierManager.recoverTokens(mockToken.target, await accounts_.solver.getAddress());
